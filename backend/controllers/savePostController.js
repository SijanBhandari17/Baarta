...
const savePost = async (req, res) => {
  ...
  try {
    ...
    if (!req.body?.postId)
      return res
        .status(400)
        .json({ error: "missing postId in the request header" });
    ...
    if (!foundPost) {
      ...
      return res
        .status(404)
        .json({ error: "the post is either deleted or removed" });
    }

    const findDuplicate = foundUser.save_post.find(
    ...
    const foundAuthor = await User.findOne({ _id: foundPost.author_id })
      .session(session)
      .exec();

    const foundAuthorProfilePic = await profilePicModel
      .findOne({ userId: foundAuthor._id })
      .session(session)
      .exec();

    const toSendResult = {
      ...foundPost.toObject(),
      authorName: foundAuthor.username,
      authorEmail: foundAuthor.email,
      authorProfilePicLink:
        foundAuthorProfilePic?.profilePicLink ||
    ...
    res
      .status(201)
      .json({ message: "successfully saved posts", body: toSendResult });
  } catch (err) {
    await session.abortTransaction();
...
const getSavedPost = async (req, res) => {
  ...
  try {
    ...
    const toSendResult = await Promise.all(
      savedPostIdArray.map(async (item) => {
        const findPost = await Post.findOne({ _id: item })
          .session(session)
          .exec();
        const authorId = await User.findOne({ _id: findPost.author_id })
          .session(session)
          .exec();
        const findParent = await Forum.findOne({ _id: findPost.parent_forum })
          .session(session)
          .exec();
        const authorProfilePic = await Profile.findOne({ userId: authorId })
          .session(session)
          .exec();
...
const deleteSavedPost = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "your saved post was deleted", body: result });
  } catch (err) {
    await session.abortTransaction();
